...
pom_modify_script=$currentDir/../starts-pom-modify/modify-project.sh
...
do
  ...
  do 
    ...
    if [[ ! -d ${inputProj}/${slug}/${module}/.starts-${sec_short_sha} ]]; then
    (
      cd ${inputProj}/${slug}
      find -name ".starts" | xargs rm -rf
      git stash
      ...
      if [[ ${slug} = "apache/incubator-dubbo" ]]; then
        if [ ${module} = "dubbo-common" -o ${module} = "dubbo-config/dubbo-config-spring" ]; then
          mvn starts:starts ${PL0} ${MVNOPTIONS}
        else
          mvn starts:starts ${PL} ${MVNOPTIONS}
        fi
      else
        mvn starts:starts ${PL} ${MVNOPTIONS}
      fi
      ...
      mv ${inputProj}/${slug}/${module}/.starts ${inputProj}/${slug}/${module}/.starts-${sec_short_sha}
      cd ${module}
      ls target/surefire-reports/*.xml | cut -d'/' -f3 | sed 's/.xml//g' | sed 's/TEST-//g' > ${inputProj}/${slug}/${module}/.starts-${sec_short_sha}/selected-tests
      rm -rf target
    )
    fi
    firstsha=${secondsha}
