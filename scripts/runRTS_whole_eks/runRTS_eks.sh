...
do
  ...
  do 
    ...
    if [[ ! -d ${inputProj}/${slug}/${module}/.dtfixingtools_eks_plus_${sec_short_sha} ]]; then
      (
        ...
        if [[ ${slug} = "apache/incubator-dubbo" ]]; then
          if [[ ${module} != "dubbo-cluster" ]]; then
            mvn testrunner:testplugin ${PL0} ${MVNOPTIONS} ${IDFLAKIESOPTIONS} ${SELECTMOREOPTION} ${DONOTDETECT} ${EKSTAZIOPTION}
          else
            mvn testrunner:testplugin ${PL} ${MVNOPTIONS} ${IDFLAKIESOPTIONS} ${SELECTMOREOPTION} ${DONOTDETECT} ${EKSTAZIOPTION}
          fi
        else
          mvn testrunner:testplugin ${PL} ${MVNOPTIONS} ${IDFLAKIESOPTIONS} ${SELECTMOREOPTION} ${DONOTDETECT} ${EKSTAZIOPTION}
        fi
        ...
        if [[ ${slug} = "apache/incubator-dubbo" ]]; then
          if [[ ${module} != "dubbo-cluster" ]]; then
            mvn testrunner:testplugin ${PL0} ${MVNOPTIONS} ${IDFLAKIESOPTIONS} ${SELECTMOREOPTION} ${EKSTAZIOPTION} ${EKSTAZISELECTEDTESTS} ${EKSTAZIDEPENDENCIES}
          else
            mvn testrunner:testplugin ${PL} ${MVNOPTIONS} ${IDFLAKIESOPTIONS} ${SELECTMOREOPTION} ${EKSTAZIOPTION} ${EKSTAZISELECTEDTESTS} ${EKSTAZIDEPENDENCIES}
          fi
        else
          mvn testrunner:testplugin ${PL} ${MVNOPTIONS} ${IDFLAKIESOPTIONS} ${SELECTMOREOPTION} ${EKSTAZIOPTION} ${EKSTAZISELECTEDTESTS} ${EKSTAZIDEPENDENCIES}
        fi
        mv $module/.dtfixingtools $module/.dtfixingtools_eks_plus_${sec_short_sha}
      )
    fi
    firstsha=${secondsha}
  done
