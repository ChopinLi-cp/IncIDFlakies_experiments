...
do
  ...
  do 
    ...
    if [[ ! -d ${inputProj}/${slug}/${module}/.dtfixingtools_starts_plus_${sec_short_sha} ]]; then
      (
        ...
        if [[ ${slug} = "apache/incubator-dubbo" ]]; then
          if [ ${module} = "dubbo-common" -o ${module} = "dubbo-config/dubbo-config-spring" ]; then
            mvn testrunner:testplugin ${PL0} ${MVNOPTIONS} ${IDFLAKIESOPTIONS} ${SELECTMOREOPTION} ${STARTSSELECTEDTESTS} ${STARTSDEPENDENCIES}
          else
            mvn testrunner:testplugin ${PL} ${MVNOPTIONS} ${IDFLAKIESOPTIONS} ${SELECTMOREOPTION} ${STARTSSELECTEDTESTS} ${STARTSDEPENDENCIES}
          fi
        else
          mvn testrunner:testplugin ${PL} ${MVNOPTIONS} ${IDFLAKIESOPTIONS} ${SELECTMOREOPTION} ${STARTSSELECTEDTESTS} ${STARTSDEPENDENCIES}
        fi
        mv $module/.dtfixingtools $module/.dtfixingtools_starts_plus_${sec_short_sha}
      )
    fi
    firstsha=${secondsha}
  done
